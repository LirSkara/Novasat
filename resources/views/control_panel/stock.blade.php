...
<div id="container" class="container-fluid px-admin-laptop px-admin-tel py-2 mb-3 pe-lg-5 mx-0">
    <div class="d-flex">
        <h1 class="mt-1">Акция</h1>
    </div>
    <div class="d-flex gap-3 mt-3 flex-100" id="stock">
        <div v-for="(item, ib) in stock" :key="item" class="d-flex flex-column bg-light shadow-sm rounded-3 px-3 pt-2 pb-3 cp-cart">
            <div class="d-flex">
                ...
                <div class="dropdown ms-auto">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <li><button class="dropdown-item" data-mdb-toggle="modal" :data-mdb-target="'#editstock' + item.id"><i class="far fa-edit me-2"></i>Редактировать</button></li>
                    </ul>
                </div>
            </div>
            <div class="d-flex flex-column rounded-3 text-white px-3 pt-4" :style="'background-image: url(/storage/stock/' + item.image + '); height: 200px; background-size: cover;'">
                <h4>@{{item.name}}</h4>
                <p>@{{item.description}}</p>
                <div>
                    <button class="btn bg-white text-warning">@{{item.btn_text}}</button>
                </div>
            </div>
            
            <!-- Start Modal Edit Stock -->
            <div class="modal fade" :id="'editstock' + item.id" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form :id="'formEditStock' + ib" class="d-flex flex-column">
                            @csrf
                                ...
                                <button v-on:click="edit_stock(ib)" class="btn btn-lg btn-dark"><i class="far fa-edit me-2"></i>Редактировать</button>
                            </form>
                        </div>
                    </div> 
                </div>
...
const Stock = {
    ...
    methods: {
        edit_stock(ib) {
            ...
            var prom = this.stock
            var e_item = prom[ib]
            var formDataEdit = new FormData($(`#formEditStock${ib}`)[0]);
            $.ajax({
                type: "POST", //Метод отправки
                url: `/edit_stock/${e_item.id}`, //путь до php фаила отправителя
...
Vue.createApp(Stock).mount('#stock')
</script>
@endsection